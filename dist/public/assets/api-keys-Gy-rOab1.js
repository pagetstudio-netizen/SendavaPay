...
}`})]}),e.jsxs("div",{id:"s-retry-payment",className:"scroll-mt-4 mb-14",children:[e.jsx("h3",{className:"text-lg font-semibold mb-1",children:"Réessayer un paiement échoué"}),e.jsxs("p",{className:"text-sm text-muted-foreground mb-3",children:["Réinitialise le statut d'une transaction ",e.jsx("code",{className:"bg-muted px-1 rounded text-xs",children:"failed"})," en ",e.jsx("code",{className:"bg-muted px-1 rounded text-xs",children:"pending"}),"pour permettre une nouvelle tentative. Appelez ensuite ",e.jsx("code",{className:"bg-muted px-1 rounded text-xs",children:"POST /initiate-payment"})," avec le même ",e.jsx("code",{className:"bg-muted px-1 rounded text-xs",children:"paymentToken"}),". Pas de clé SDK requise — endpoint CORS public."]}),e.jsx(d,{method:"POST",path:"/api/sdk/v1/retry-payment"}),e.jsx("h4",{className:"text-xs font-semibold uppercase tracking-wide text-muted-foreground mt-5 mb-2",children:"Corps de la requête"}),e.jsx(f,{params:[{name:"paymentToken",type:"string",required:!0,description:"paymentToken de la transaction échouée"}]}),e.jsx(a,{language:"javascript",code:`const res = await fetch(
...
}`}),e.jsxs(p,{type:"tip",title:"Idempotence",children:["Vérifiez toujours si la ",e.jsx("code",{children:"reference"})," a déjà été traitée avant d'agir — SendavaPay peut renvoyer le même événement en cas d'échec de votre endpoint."]})]}),e.jsxs("div",{id:"s-wh-signature",className:"scroll-mt-4 mb-14",children:[e.jsx("h3",{className:"text-lg font-semibold mb-1",children:"Vérification HMAC"}),e.jsx(p,{type:"danger",title:"Toujours vérifier la signature",children:"Ne traitez jamais un webhook sans vérifier sa signature. Un webhook non vérifié peut être forgé pour simuler un paiement réussi."}),e.jsx(a,{language:"bash — calcul de la signature",code:`HMAC_SHA256(key=WEBHOOK_SECRET, data=JSON.stringify(body))
...
}`}),e.jsx("div",{className:"overflow-x-auto rounded-lg border text-sm mt-4",children:e.jsxs("table",{className:"w-full",children:[e.jsx("thead",{className:"bg-muted/70",children:e.jsxs("tr",{children:[e.jsx("th",{className:"text-left p-3 font-semibold text-xs uppercase tracking-wide text-muted-foreground",children:"HTTP"}),e.jsx("th",{className:"text-left p-3 font-semibold text-xs uppercase tracking-wide text-muted-foreground",children:"Code"}),e.jsx("th",{className:"text-left p-3 font-semibold text-xs uppercase tracking-wide text-muted-foreground",children:"Description"})]})}),e.jsx("tbody",{children:[["400","INVALID_REQUEST","Paramètres manquants ou malformés"],["400","DUPLICATE_REFERENCE","externalReference déjà utilisée pour un paiement actif"],["400","INSUFFICIENT_BALANCE","Solde insuffisant sur le wallet pour le retrait"],["400","UNSUPPORTED_COUNTRY","Code pays non pris en charge"],["400","WALLET_NOT_FOUND","Wallet du pays introuvable sur votre compte"],["400","INVALID_PHONE_FORMAT","Numéro de téléphone invalide — format E.164 requis (+22890…)"],["400","OPERATOR_COUNTRY_MISMATCH","Opérateur incompatible avec le pays demandé"],["400","PAYOUT_OPERATOR_OFFLINE","Opérateur en maintenance — retraits temporairement indisponibles"],["400","AMOUNT_TOO_LOW","Montant inférieur au minimum autorisé (100)"],["400","AMOUNT_TOO_HIGH","Montant supérieur au maximum autorisé (5 000 000)"],["409","DUPLICATE_WITHDRAWAL","Un retrait avec cet externalReference existe déjà"],["401","UNAUTHORIZED","Header Authorization manquant ou malformé"],["401","INVALID_API_KEY","Clé SDK invalide ou inexistante"],["403","API_KEY_INACTIVE","Clé SDK désactivée"],["403","SDK_NOT_ENABLED","Accès SDK non activé — contactez le support"],["403","ACCOUNT_NOT_VERIFIED","KYC non validé"],["404","NOT_FOUND","Transaction introuvable pour cette référence"],["410","TOKEN_EXPIRED","paymentToken expiré (30 min dépassées)"],["503","API_MAINTENANCE","API temporairement en maintenance"]].map(([s,n,i],g)=>e.jsxs("tr",{className:"border-t hover:bg-muted/20",children:[e.jsx("td",{className:"p-3 font-mono text-xs text-muted-foreground",children:s}),e.jsx("td",{className:"p-3 font-mono text-xs text-primary",children:n}),e.jsx("td",{className:"p-3 text-xs text-muted-foreground",children:i})]},g))})]})})]}),e.jsxs("section",{id:"s-rate",className:"pb-16",children:[e.jsx("h2",{className:"text-2xl font-bold border-b pb-3 mb-6",children:"Rate Limiting"}),e.jsx("div",{className:"grid sm:grid-cols-3 gap-3 mb-6",children:[["100 req / min","Par clé SDK"],["5 secondes","Intervalle polling recommandé"],["Retry 429","Backoff exponentiel"]].map(([s,n])=>e.jsxs("div",{className:"p-4 rounded-lg border bg-muted/30 text-center",children:[e.jsx("p",{className:"text-lg font-bold text-primary",children:s}),e.jsx("p",{className:"text-xs text-muted-foreground",children:n})]},s))}),e.jsx(a,{language:"json — réponse HTTP 429",code:`HTTP/1.1 429 Too Many Requests
{
  ...
  "code":    "RATE_LIMITED"
}`}),e.jsx(p,{type:"tip",children:"Pour le polling de statut, utilisez un intervalle de 5 secondes minimum. Implémentez un backoff exponentiel sur les réponses 429."})]})]})]})}function Jt(){const{user:o,isLoading:l}=Fe(),{toast:r}=De(),[m,y]=c.useState(""),[S,O]=c.useState(""),[h,D]=c.useState(""),[U,s]=c.useState(""),[n,i]=c.useState(null),[g,I]=c.useState(null),[E,J]=c.useState(null),[he,$]=c.useState(!1),[ge,Z]=c.useState(null),[je,Q]=c.useState(""),[fe,ee]=c.useState(""),[L,B]=c.useState(""),[te,be]=c.useState(!1),{data:_,isLoading:Ae}=ie({queryKey:["/api/api-maintenance-status"],refetchInterval:1e4}),{data:se,isLoading:Pe}=ie({queryKey:["/api/user/api-permissions"],enabled:!!(o!=null&&o.isVerified)}),{data:ye=[],isLoading:ae}=ie({queryKey:["/api/api-keys"],enabled:!!(o!=null&&o.isVerified)&&!(_!=null&&_.enabled)}),re=de({mutationFn:async t=>(await le("POST","/api/api-keys",t)).json(),onSuccess:()=>{ce.invalidateQueries({queryKey:["/api/api-keys"]}),y(""),O(""),D(""),s(""),i(null),$(!1),r({title:"Clé créée",description:"Votre nouvelle clé API a été créée avec succès"})},onError:t=>{r({title:"Erreur",description:t.message||"Impossible de créer la clé",variant:"destructive"})}}),ne=de({mutationFn:async t=>{await le("DELETE",`/api/api-keys/${t}`)},onSuccess:()=>{ce.invalidateQueries({queryKey:["/api/api-keys"]}),r({title:"Clé supprimée"})},onError:()=>{r({title:"Erreur",description:"Impossible de supprimer la clé",variant:"destructive"})}}),Re=async t=>{const x=new FormData;x.append("image",t);const j=await fetch("/api/upload/product-image",{method:"POST",body:x,credentials:"include"});if(!j.ok)throw new Error("Erreur lors de l'upload du logo");return(await j.json()).imageUrl},Ee=async t=>{var j;const x=(j=t.target.files)==null?void 0:j[0];if(x){be(!0);try{const w=await Re(x);B(w),r({title:"Logo chargé",description:"Le logo sera enregistré avec les autres modifications"})}catch{r({title:"Erreur",description:"Impossible de charger le logo",variant:"destructive"})}finally{be(!1)}}},Ne=de({mutationFn:async({id:t,webhookUrl:x,redirectUrl:j,logoUrl:w})=>(await le("PATCH",`/api/api-keys/${t}`,{webhookUrl:x||null,redirectUrl:j||null,logoUrl:w||null})).json(),onSuccess:()=>{ce.invalidateQueries({queryKey:["/api/api-keys"]}),Z(null),r({title:"Clé mise à jour",description:"Les URLs ont été enregistrées"})},onError:()=>{r({title:"Erreur",description:"Impossible de mettre à jour la clé",variant:"destructive"})}}),_e=t=>{Z(t.id),Q(t.webhookUrl||""),ee(t.redirectUrl||""),B(t.logoUrl||"")},ve=()=>{Z(null),Q(""),ee(""),B("")},we=t=>{navigator.clipboard.writeText(t),I(t),r({title:"Copié"}),setTimeout(()=>I(null),2e3)},qe=()=>{if(!m.trim()){r({title:"Erreur",description:"Veuillez entrer un nom pour la clé",variant:"destructive"});return}if(!n){r({title:"Erreur",description:"Veuillez sélectionner un type d'API",variant:"destructive"});return}re.mutate({name:m.trim(),appName:S.trim()||void 0,webhookUrl:h.trim()||void 0,redirectUrl:U.trim()||void 0,apiType:n})};if(l||Ae)return e.jsx(X,{children:e.jsx("div",{className:"flex items-center justify-center min-h-[60vh]",children:e.jsx(k,{className:"h-8 w-8 animate-spin text-primary"})})});if(_!=null&&_.enabled)return e.jsx(X,{children:e.jsx("div",{className:"flex items-center justify-center min-h-[60vh] p-4",children:e.jsxs(N,{className:"max-w-lg w-full text-center",children:[e.jsxs(T,{children:[e.jsx("div",{className:"mx-auto mb-4 h-16 w-16 rounded-full bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center",children:e.jsx(tt,{className:"h-8 w-8 text-orange-600 dark:text-orange-400"})}),e.jsx(C,{className:"text-2xl",children:"API en maintenance"}),e.jsx(P,{className:"text-base",children:"L'API est temporairement indisponible"})]}),e.jsx(v,{children:e.jsx(G,{href:"/dashboard",children:e.jsxs(u,{variant:"outline",children:[e.jsx(me,{className:"h-4 w-4 mr-2"}),"Retour au tableau de bord"]})})})]})})});if(!(o!=null&&o.isVerified))return e.jsx(X,{children:e.jsxs("div",{className:"space-y-6 p-6",children:[e.jsxs("div",{children:[e.jsx("h1",{className:"text-2xl font-bold",children:"API de Paiement"}),e.jsx("p",{className:"text-muted-foreground",children:"Intégrez SendavaPay dans vos applications"})]}),e.jsxs(N,{children:[e.jsxs(T,{children:[e.jsxs(C,{className:"flex items-center gap-2",children:[e.jsx(Ce,{className:"h-5 w-5 text-yellow-500"}),"Vérification requise"]}),e.jsx(P,{children:"Votre compte doit être vérifié pour accéder à l'API"})]}),e.jsxs(v,{className:"space-y-4",children:[e.jsx("p",{className:"text-muted-foreground",children:"Pour des raisons de sécurité, seuls les comptes vérifiés peuvent accéder à l'API SendavaPay."}),e.jsx(G,{href:"/dashboard/kyc",children:e.jsxs(u,{children:[e.jsx(Ce,{className:"h-4 w-4 mr-2"}),"Vérifier mon compte"]})})]})]})]})});const K=(se==null?void 0:se.apiSdkEnabled)??!1,A=ye.filter(t=>t.apiType==="sdk"),b=ye.filter(t=>t.apiType==="redirect"),oe=({keyItem:t})=>{const x=ge===t.id,j=Ne.isPending&&ge===t.id;return e.jsxs("div",{className:"flex flex-col gap-3 p-4 border rounded-lg","data-testid":`api-key-${t.id}`,children:[e.jsxs("div",{className:"flex flex-col sm:flex-row sm:items-start justify-between gap-3",children:[e.jsxs("div",{className:"space-y-1 flex-1 min-w-0",children:[e.jsxs("div",{className:"flex items-center gap-2 flex-wrap",children:[t.logoUrl&&e.jsx("img",{src:t.logoUrl,alt:"Logo",className:"h-6 w-6 rounded object-cover border"}),e.jsx("span",{className:"font-medium",children:t.name}),e.jsx(H,{variant:t.isActive?"default":"secondary",children:t.isActive?"Active":"Inactive"}),e.jsx(H,{variant:"outline",className:t.apiType==="sdk"?"border-purple-400 text-purple-700 dark:text-purple-300":"",children:t.apiType==="sdk"?"SDK":"Redirection"})]}),t.appName&&e.jsxs("p",{className:"text-sm text-muted-foreground",children:["Application : ",t.appName]}),e.jsx("code",{className:"text-sm text-muted-foreground font-mono block truncate",children:t.apiKey}),e.jsxs("p",{className:"text-xs text-muted-foreground",children:["Créée le ",new Date(t.createdAt).toLocaleDateString("fr-FR"),t.requestCount>0&&` • ${t.requestCount} requêtes`]})]}),e.jsxs("div",{className:"flex gap-2 flex-shrink-0",children:[e.jsx(u,{variant:"outline",size:"sm",onClick:()=>we(t.apiKey),"data-testid":`button-copy-${t.id}`,children:g===t.apiKey?e.jsx(F,{className:"h-4 w-4"}):e.jsx(Y,{className:"h-4 w-4"})}),e.jsx(u,{variant:"outline",size:"sm",onClick:()=>x?ve():_e(t),"data-testid":`button-edit-${t.id}`,children:x?e.jsx(Ue,{className:"h-4 w-4"}):e.jsx(We,{className:"h-4 w-4"})}),e.jsx(u,{variant:"outline",size:"sm",onClick:()=>J(t),disabled:ne.isPending,"data-testid":`button-delete-${t.id}`,children:e.jsx(Oe,{className:"h-4 w-4 text-destructive"})})]})]}),x&&e.jsxs("div",{className:"border-t pt-3 space-y-3 bg-muted/20 -mx-4 px-4 pb-3 rounded-b-lg",children:[e.jsx("p",{className:"text-xs font-semibold uppercase tracking-wide text-muted-foreground",children:"Modifier la clé"}),e.jsxs("div",{className:"space-y-1",children:[e.jsxs(R,{className:"text-xs flex items-center gap-1.5",children:[e.jsx(Je,{className:"h-3 w-3"}),"Logo de l'application"]}),e.jsxs("div",{className:"flex items-center gap-3",children:[L&&e.jsx("img",{src:L,alt:"Logo",className:"h-10 w-10 rounded object-cover border"}),e.jsxs("label",{htmlFor:`logo-${t.id}`,className:"cursor-pointer inline-flex items-center gap-1.5 h-8 px-3 text-xs rounded-md border border-input bg-background hover:bg-accent hover:text-accent-foreground",children:[te?e.jsx(k,{className:"h-3 w-3 animate-spin"}):e.jsx(Ze,{className:"h-3 w-3"}),L?"Changer":"Importer un logo"]}),e.jsx("input",{id:`logo-${t.id}`,type:"file",accept:"image/*",className:"hidden",onChange:Ee,disabled:te,"data-testid":`input-logo-${t.id}`}),L&&e.jsx("button",{type:"button",onClick:()=>B(""),className:"text-xs text-muted-foreground hover:text-destructive",children:"Supprimer"})]}),e.jsx("p",{className:"text-xs text-muted-foreground",children:"Ce logo s'affiche sur la page de paiement"})]}),e.jsxs("div",{className:"space-y-1",children:[e.jsxs(R,{htmlFor:`wh-${t.id}`,className:"text-xs flex items-center gap-1.5",children:[e.jsx(Se,{className:"h-3 w-3"}),"URL de webhook"]}),e.jsx(q,{id:`wh-${t.id}`,type:"url",placeholder:"https://monsite.com/webhooks/sendavapay",value:je,onChange:w=>Q(w.target.value),className:"h-8 text-sm","data-testid":`input-edit-webhook-${t.id}`}),e.jsx("p",{className:"text-xs text-muted-foreground",children:"Laisser vide pour désactiver les webhooks"})]}),e.jsxs("div",{className:"space-y-1",children:[e.jsxs(R,{htmlFor:`rd-${t.id}`,className:"text-xs flex items-center gap-1.5",children:[e.jsx(xe,{className:"h-3 w-3"}),"URL de redirection (après paiement)"]}),e.jsx(q,{id:`rd-${t.id}`,type:"url",placeholder:"https://monsite.com/paiement/succes",value:fe,onChange:w=>ee(w.target.value),className:"h-8 text-sm","data-testid":`input-edit-redirect-${t.id}`})]}),e.jsxs("div",{className:"flex gap-2 pt-1",children:[e.jsxs(u,{size:"sm",onClick:()=>Ne.mutate({id:t.id,webhookUrl:je,redirectUrl:fe,logoUrl:L}),disabled:j||te,"data-testid":`button-save-${t.id}`,children:[j?e.jsx(k,{className:"h-3 w-3 mr-1.5 animate-spin"}):e.jsx(Qe,{className:"h-3 w-3 mr-1.5"}),"Enregistrer"]}),e.jsx(u,{size:"sm",variant:"outline",onClick:ve,disabled:j,"data-testid":`button-cancel-edit-${t.id}`,children:"Annuler"})]})]}),!x&&(t.redirectUrl||t.webhookUrl)&&e.jsxs("div",{className:"space-y-2 border-t pt-3",children:[e.jsxs("div",{className:"flex flex-wrap gap-4 text-xs",children:[t.redirectUrl&&e.jsxs("div",{className:"flex items-center gap-1 text-muted-foreground min-w-0",children:[e.jsx(xe,{className:"h-3 w-3 flex-shrink-0"}),e.jsx("span",{className:"flex-shrink-0",children:"Redirection:"}),e.jsx("span",{className:"font-mono truncate max-w-[200px]",children:t.redirectUrl})]}),t.webhookUrl&&e.jsxs("div",{className:"flex items-center gap-1 text-muted-foreground min-w-0",children:[e.jsx(et,{className:"h-3 w-3 flex-shrink-0"}),e.jsx("span",{className:"flex-shrink-0",children:"Webhook:"}),e.jsx("span",{className:"font-mono truncate max-w-[200px]",children:t.webhookUrl})]})]}),t.webhookSecret&&e.jsxs("div",{className:"flex items-center gap-2 text-xs",children:[e.jsx("span",{className:"text-muted-foreground",children:"Secret:"}),e.jsx("code",{className:"font-mono text-muted-foreground bg-muted px-2 py-0.5 rounded truncate max-w-[200px]",children:t.webhookSecret}),e.jsx(u,{variant:"ghost",size:"sm",className:"h-6 w-6 p-0",onClick:()=>we(t.webhookSecret),"data-testid":`button-copy-secret-${t.id}`,children:g===t.webhookSecret?e.jsx(F,{className:"h-3 w-3"}):e.jsx(Y,{className:"h-3 w-3"})})]})]})]})};return e.jsxs(X,{children:[e.jsxs("div",{className:"space-y-6 p-6",children:[e.jsxs("div",{children:[e.jsx("h1",{className:"text-2xl font-bold",children:"API de Paiement"}),e.jsx("p",{className:"text-muted-foreground",children:"Gérez vos clés d'intégration SendavaPay"})]}),K&&!n&&e.jsxs(N,{children:[e.jsxs(T,{children:[e.jsxs(C,{className:"flex items-center gap-2",children:[e.jsx(ue,{className:"h-5 w-5"}),"Créer une clé API"]}),e.jsx(P,{children:"Choisissez le type d'intégration adapté à votre projet"})]}),e.jsx(v,{children:e.jsxs("div",{className:"grid sm:grid-cols-2 gap-4",children:[e.jsxs("button",{className:"p-5 border-2 rounded-xl text-left hover:border-purple-500 hover:bg-purple-50 dark:hover:bg-purple-900/10 transition-all group",onClick:()=>i("sdk"),children:[e.jsxs("div",{className:"flex items-center gap-3 mb-3",children:[e.jsx("div",{className:"h-10 w-10 rounded-lg bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center group-hover:bg-purple-200 transition-colors",children:e.jsx(z,{className:"h-5 w-5 text-purple-600 dark:text-purple-400"})}),e.jsxs("div",{children:[e.jsx("p",{className:"font-semibold",children:"API SDK"}),e.jsx("p",{className:"text-xs text-muted-foreground",children:"Intégration complète"})]})]}),e.jsx("ul",{className:"text-sm text-muted-foreground space-y-1",children:["Encaissement Mobile Money (10 pays)","Retrait automatique pay-out","Soldes wallets par pays","Webhooks signés HMAC"].map(t=>e.jsxs("li",{className:"flex items-center gap-2",children:[e.jsx(F,{className:"h-3 w-3 text-green-500 flex-shrink-0"}),t]},t))})]}),e.jsxs("button",{className:"p-5 border-2 rounded-xl text-left hover:border-primary hover:bg-primary/5 transition-all group",onClick:()=>i("redirect"),children:[e.jsxs("div",{className:"flex items-center gap-3 mb-3",children:[e.jsx("div",{className:"h-10 w-10 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center group-hover:bg-blue-200 transition-colors",children:e.jsx(xe,{className:"h-5 w-5 text-blue-600 dark:text-blue-400"})}),e.jsxs("div",{children:[e.jsx("p",{className:"font-semibold",children:"API Redirection"}),e.jsx("p",{className:"text-xs text-muted-foreground",children:"Simple et rapide"})]})]}),e.jsx("ul",{className:"text-sm text-muted-foreground space-y-1",children:["Liens de paiement générés","Redirection après paiement","Notifications webhook","Intégration sans SDK"].map(t=>e.jsxs("li",{className:"flex items-center gap-2",children:[e.jsx(F,{className:"h-3 w-3 text-green-500 flex-shrink-0"}),t]},t))})]})]})})]}),(K&&n||!K&&he)&&e.jsxs(N,{children:[e.jsxs(T,{children:[e.jsxs("div",{className:"flex items-center justify-between",children:[e.jsx(C,{className:"flex items-center gap-2",children:n==="sdk"?e.jsxs(e.Fragment,{children:[e.jsx(z,{className:"h-5 w-5 text-purple-600"}),"Nouvelle clé API SDK"]}):e.jsxs(e.Fragment,{children:[e.jsx(V,{className:"h-5 w-5 text-primary"}),"Nouvelle clé API"]})}),K?e.jsxs(u,{variant:"ghost",size:"sm",onClick:()=>i(null),"data-testid":"button-back-type",children:[e.jsx(me,{className:"h-4 w-4 mr-1"}),"Changer"]}):e.jsxs(u,{variant:"ghost",size:"sm",onClick:()=>$(!1),"data-testid":"button-cancel-create",children:[e.jsx(me,{className:"h-4 w-4 mr-1"}),"Annuler"]})]}),e.jsx(P,{children:n==="sdk"?"Clé pour l'intégration SDK complète avec retrait automatique":"Clé pour la création de liens de paiement avec redirection"})]}),e.jsxs(v,{className:"space-y-4",children:[e.jsxs("div",{className:"grid sm:grid-cols-2 gap-4",children:[e.jsxs("div",{className:"space-y-2",children:[e.jsx(R,{htmlFor:"keyName",children:"Nom de la clé *"}),e.jsx(q,{id:"keyName",placeholder:"Ex: Mon site e-commerce",value:m,onChange:t=>y(t.target.value),"data-testid":"input-key-name"})]}),e.jsxs("div",{className:"space-y-2",children:[e.jsx(R,{htmlFor:"appName",children:"Nom de l'application"}),e.jsx(q,{id:"appName",placeholder:"Ex: MaBoutique.com",value:S,onChange:t=>O(t.target.value),"data-testid":"input-app-name"})]})]}),e.jsxs("div",{className:"space-y-2",children:[e.jsx(R,{htmlFor:"redirectUrl",children:"URL de redirection (après paiement)"}),e.jsx(q,{id:"redirectUrl",type:"url",placeholder:"https://monsite.com/paiement/succes",value:U,onChange:t=>s(t.target.value),"data-testid":"input-redirect-url"})]}),e.jsxs("div",{className:"space-y-2",children:[e.jsx(R,{htmlFor:"webhookUrl",children:e.jsxs("div",{className:"flex items-center gap-2",children:[e.jsx(Se,{className:"h-4 w-4"}),"URL de webhook"]})}),e.jsx(q,{id:"webhookUrl",type:"url",placeholder:"https://monsite.com/webhooks/sendavapay",value:h,onChange:t=>D(t.target.value),"data-testid":"input-webhook-url"}),e.jsx("p",{className:"text-xs text-muted-foreground",children:"SendavaPay enverra une notification POST sécurisée à cette URL lors de chaque paiement"})]}),e.jsxs(u,{onClick:qe,disabled:re.isPending,className:n==="sdk"?"bg-purple-600 hover:bg-purple-700":"","data-testid":"button-create-key",children:[re.isPending?e.jsx(k,{className:"h-4 w-4 mr-2 animate-spin"}):e.jsx(V,{className:"h-4 w-4 mr-2"}),"Générer la clé"]})]})]}),!Pe&&e.jsx(e.Fragment,{children:K?e.jsxs(Le,{defaultValue:A.length>0?"sdk":"redirect",children:[e.jsxs(Ke,{children:[e.jsxs(ke,{value:"sdk",className:"gap-2",children:[e.jsx(z,{className:"h-4 w-4"}),"Clés SDK",A.length>0&&e.jsx(H,{variant:"secondary",className:"ml-1 h-5 text-xs",children:A.length})]}),e.jsxs(ke,{value:"redirect",className:"gap-2",children:[e.jsx(pe,{className:"h-4 w-4"}),"Clés Redirection",b.length>0&&e.jsx(H,{variant:"secondary",className:"ml-1 h-5 text-xs",children:b.length})]})]}),e.jsx(Te,{value:"sdk",className:"space-y-4 mt-4",children:e.jsxs(N,{children:[e.jsxs(T,{children:[e.jsxs(C,{className:"flex items-center gap-2 text-base",children:[e.jsx(z,{className:"h-4 w-4 text-purple-600"}),"Clés API SDK"]}),e.jsxs(P,{children:[A.length," clé",A.length!==1?"s":""," SDK"]})]}),e.jsx(v,{children:ae?e.jsx("div",{className:"flex justify-center py-8",children:e.jsx(k,{className:"h-6 w-6 animate-spin text-muted-foreground"})}):A.length===0?e.jsxs("div",{className:"text-center py-8 text-muted-foreground",children:[e.jsx(z,{className:"h-12 w-12 mx-auto mb-3 opacity-50"}),e.jsx("p",{children:"Aucune clé SDK créée"}),e.jsx("p",{className:"text-sm",children:'Cliquez sur "Créer une clé API" pour commencer'})]}):e.jsx("div",{className:"space-y-3",children:A.map(t=>e.jsx(oe,{keyItem:t},t.id))})})]})}),e.jsxs(Te,{value:"redirect",className:"space-y-4 mt-4",children:[e.jsxs(N,{children:[e.jsxs(T,{children:[e.jsxs(C,{className:"flex items-center gap-2 text-base",children:[e.jsx(pe,{className:"h-4 w-4 text-blue-600"}),"Clés API Redirection"]}),e.jsxs(P,{children:[b.length," clé",b.length!==1?"s":""," Redirection"]})]}),e.jsx(v,{children:ae?e.jsx("div",{className:"flex justify-center py-8",children:e.jsx(k,{className:"h-6 w-6 animate-spin text-muted-foreground"})}):b.length===0?e.jsxs("div",{className:"text-center py-8 text-muted-foreground",children:[e.jsx(pe,{className:"h-12 w-12 mx-auto mb-3 opacity-50"}),e.jsx("p",{children:"Aucune clé créée"})]}):e.jsx("div",{className:"space-y-3",children:b.map(t=>e.jsx(oe,{keyItem:t},t.id))})})]}),e.jsxs(N,{children:[e.jsx(T,{children:e.jsxs(C,{className:"flex items-center gap-2 text-base",children:[e.jsx(W,{className:"h-4 w-4"}),"Documentation"]})}),e.jsx(v,{children:e.jsx(G,{href:"/docs",children:e.jsxs(u,{variant:"outline","data-testid":"button-docs-redirect",children:[e.jsx(W,{className:"h-4 w-4 mr-2"}),"Voir la documentation"]})})})]})]})]}):e.jsxs("div",{className:"space-y-4",children:[e.jsxs(N,{children:[e.jsx(T,{children:e.jsxs("div",{className:"flex items-center justify-between",children:[e.jsxs("div",{children:[e.jsxs(C,{className:"flex items-center gap-2 text-base",children:[e.jsx(V,{className:"h-4 w-4 text-primary"}),"Mes clés API"]}),e.jsxs(P,{children:[b.length," clé",b.length!==1?"s":""]})]}),!he&&e.jsxs(u,{size:"sm",onClick:()=>{$(!0),i("redirect")},"data-testid":"button-new-key",children:[e.jsx(ue,{className:"h-4 w-4 mr-2"}),"Nouvelle clé"]})]})}),e.jsx(v,{children:ae?e.jsx("div",{className:"flex justify-center py-8",children:e.jsx(k,{className:"h-6 w-6 animate-spin text-muted-foreground"})}):b.length===0?e.jsxs("div",{className:"text-center py-8 text-muted-foreground",children:[e.jsx(V,{className:"h-12 w-12 mx-auto mb-3 opacity-50"}),e.jsx("p",{children:"Aucune clé créée"}),e.jsxs(u,{className:"mt-3",size:"sm",onClick:()=>{$(!0),i("redirect")},"data-testid":"button-create-first-key",children:[e.jsx(ue,{className:"h-4 w-4 mr-2"}),"Créer ma première clé"]})]}):e.jsx("div",{className:"space-y-3",children:b.map(t=>e.jsx(oe,{keyItem:t},t.id))})})]}),e.jsx(N,{children:e.jsx(v,{className:"pt-6",children:e.jsxs("div",{className:"flex items-center gap-4",children:[e.jsx("div",{className:"h-10 w-10 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0",children:e.jsx(W,{className:"h-5 w-5 text-primary"})}),e.jsxs("div",{className:"flex-1",children:[e.jsx("p",{className:"font-medium",children:"Documentation API"}),e.jsx("p",{className:"text-sm text-muted-foreground",children:"Consultez la documentation pour intégrer SendavaPay dans votre application"})]}),e.jsx(G,{href:"/documentation",children:e.jsxs(u,{variant:"outline","data-testid":"button-docs",children:[e.jsx(W,{className:"h-4 w-4 mr-2"}),"Voir la documentation"]})})]})})})]})})]}),e.jsx(ze,{open:!!E,onOpenChange:t=>!t&&J(null),children:e.jsxs(Ie,{children:[e.jsxs($e,{children:[e.jsx(Be,{children:"Supprimer cette clé API ?"}),e.jsxs(Ge,{children:['Êtes-vous sûr de vouloir supprimer la clé "',E==null?void 0:E.name,'" ? Cette action est irréversible et toutes les intégrations utilisant cette clé cesseront de fonctionner.']})]}),e.jsxs(Xe,{children:[e.jsx(He,{children:"Annuler"}),e.jsxs(Ve,{onClick:()=>{E&&(ne.mutate(E.id),J(null))},className:"bg-destructive text-destructive-foreground hover:bg-destructive/90",children:[ne.isPending?e.jsx(k,{className:"h-4 w-4 mr-2 animate-spin"}):e.jsx(Oe,{className:"h-4 w-4 mr-2"}),"Supprimer"]})]})]})})]})}export{Yt as SdkDocumentation,Jt as default};
