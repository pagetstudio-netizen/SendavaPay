...
import {
  pgTable, pgEnum, varchar, text, integer, boolean, decimal, timestamp, serial,
...
export const exchangeStatusEnum      = pgEnum("exchange_status", ["pending", "approved", "rejected"]);
export const withdrawalStatusEnum    = pgEnum("withdrawal_status", ["pending", "processing", "approved", "rejected", "failed"]);
export const leekpayStatusEnum       = pgEnum("leekpay_status", ["pending", "processing", "completed", "failed", "cancelled", "expired"]);
export const adminNotifTypeEnum      = pgEnum("admin_notif_type", ["transaction", "kyc", "withdrawal", "user", "system"]);
export const merchantStatusEnum      = pgEnum("merchant_status", ["active", "suspended", "pending"]);
export const apiTxTypeEnum           = pgEnum("api_tx_type", ["payment", "credit", "refund", "payout"]);
export const apiTxStatusEnum         = pgEnum("api_tx_status", ["pending", "queued", "processing", "provider_pending", "completed", "failed", "reversed", "cancelled"]);
...
export const partnerTxStatusEnum     = pgEnum("partner_tx_status", ["pending", "queued", "processing", "provider_pending", "completed", "failed", "reversed", "cancelled"]);
...
export const leekpayPayments = pgTable("leekpay_payments", {
  ...
  status: leekpayStatusEnum("status").default("pending").notNull(),
  description: text("description"),
...
export const countries = pgTable("countries", {
  ...
  apiFeeRate: decimal("api_fee_rate", { precision: 5, scale: 2 }),
  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const adminNotifications = pgTable("admin_notifications", {
  id: serial("id").primaryKey(),
  type: adminNotifTypeEnum("type").notNull(),
  title: text("title").notNull(),
...
export const apiTransactions = pgTable("api_transactions", {
  ...
  externalReference: text("external_reference"),
  type: apiTxTypeEnum("type").notNull(),
  amount: decimal("amount", { precision: 15, scale: 2 }).notNull(),
  fee: decimal("fee", { precision: 15, scale: 2 }).default("0").notNull(),
  currency: varchar("currency", { length: 10 }).default("XOF").notNull(),
  status: apiTxStatusEnum("status").default("pending").notNull(),
  description: text("description"),
  customerEmail: text("customer_email"),
...
export const transfersRelations = relations(transfers, ({ one }) => ({
  sender: one(users, { fields: [transfers.senderId], references: [users.id], relationName: "sender" }),
  receiver: one(users, { fields: [transfers.receiverId], references: [users.id], relationName: "receiver" }),
}));
...
export const kycRequestsRelations = relations(kycRequests, ({ one }) => ({
  ...
  reviewer: one(users, { fields: [kycRequests.reviewedBy], references: [users.id] }),
}));
...
export const insertUserSchema = createInsertSchema(users).omit({
  id: true, createdAt: true, balance: true, isVerified: true, isBlocked: true, role: true,
});
...
export const insertKycRequestSchema = createInsertSchema(kycRequests).omit({
  id: true, createdAt: true, status: true, reviewedBy: true, reviewedAt: true, rejectionReason: true,
});
...
export const insertPaymentLinkSchema = createInsertSchema(paymentLinks).omit({
  id: true, createdAt: true, status: true, paidAt: true, payerName: true, payerEmail: true, linkCode: true,
});

export const insertApiKeySchema = createInsertSchema(apiKeys).omit({
  id: true, createdAt: true, apiKey: true, lastUsedAt: true, requestCount: true, isActive: true,
});

export const insertAuditLogSchema = createInsertSchema(auditLogs).omit({ id: true, createdAt: true });

export const insertWithdrawalRequestSchema = createInsertSchema(withdrawalRequests).omit({
  id: true, createdAt: true, status: true, rejectionReason: true, reviewedBy: true, reviewedAt: true,
});
...
export const insertMerchantSchema = createInsertSchema(merchants).omit({
  id: true, createdAt: true, apiKey: true, apiSecret: true, balance: true,
  status: true, isVerified: true, lastLoginAt: true, webhookSecret: true,
});
...
export const insertApiTransactionSchema = createInsertSchema(apiTransactions).omit({
  id: true, createdAt: true, completedAt: true, webhookSent: true,
  webhookAttempts: true, webhookLastAttempt: true,
});

export const insertMerchantWebhookSchema = createInsertSchema(merchantWebhooks).omit({
  id: true, createdAt: true, lastTriggered: true, failureCount: true,
});
...
export const partnerTransactions = pgTable("partner_transactions", {
  ...
  fee: decimal("fee", { precision: 15, scale: 2 }).default("0").notNull(),
  currency: varchar("currency", { length: 10 }).default("XOF").notNull(),
  status: partnerTxStatusEnum("status").default("pending").notNull(),
  customerName: text("customer_name"),
  customerEmail: text("customer_email"),
  customerPhone: text("customer_phone"),
  paymentMethod: text("payment_method"),
...
export const partnerLogsRelations = relations(partnerLogs, ({ one }) => ({
  partner: one(partners, { fields: [partnerLogs.partnerId], references: [partners.id] }),
}));
...
export const insertPartnerSchema = createInsertSchema(partners).omit({
  id: true, createdAt: true, apiKey: true, apiSecret: true, balance: true, status: true, lastLoginAt: true,
});
...
export const insertPartnerLogSchema = createInsertSchema(partnerLogs).omit({ id: true, createdAt: true });
