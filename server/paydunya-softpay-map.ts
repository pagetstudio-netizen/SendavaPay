 ...
 *         https://developers.paydunya.com/doc/FR/api_softpay_index
 *
 * Each entry defines:
 *  - slug         : URL slug for POST /api/v1/softpay/{slug}
...
// Slugs validated against PayDunya official documentation
const OPERATORS: SoftPayOperatorDef[] = [
  ...
  {
    // Slug confirmed: moov-burkina-faso (PayDunya docs use full country name)
    slug: "moov-burkina-faso",
    countryCode: "BF",
    currency: "XOF",
    responseType: "direct",
    requiresOtp: false,
...
const SLUG_MAP: Record<string, string> = {
  ...
  "wizall money senegal":  "wizall-money-senegal",
...
export function getSoftPaySlug(operatorName: string, countryCode: string): string | null {
  ...
  // Try "name cc" first, then name alone
  if (SLUG_MAP[`${name} ${cc}`]) return SLUG_MAP[`${name} ${cc}`];
